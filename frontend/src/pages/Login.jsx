...
export default function Login() {
  ...
  const features = [
    { icon: '🧠', title: 'AI Meal Planning', desc: 'Personalized Indian meal plans by LLaMA 3.1 AI.', tag: 'AI', tagColor: 'bg-violet-100 text-violet-700' },
    { icon: '⚖️', title: 'BMI Calculator', desc: 'Real-time BMI with personalized health insights.', tag: 'Smart', tagColor: 'bg-blue-100 text-blue-700' },
    { icon: '🔥', title: 'Calorie Tracker', desc: 'Track macros with 500+ Indian foods database.', tag: 'Live', tagColor: 'bg-orange-100 text-orange-700' },
    { icon: '💡', title: 'Smart Tips', desc: 'AI tips based on your goals and daily intake.', tag: 'Personal', tagColor: 'bg-yellow-100 text-yellow-700' },
    { icon: '📈', title: 'Progress Tracking', desc: 'Streaks, charts and milestone celebrations.', tag: 'Analytics', tagColor: 'bg-green-100 text-green-700' },
    { icon: '🎯', title: 'Goal-Based Plans', desc: 'Custom Indian diet plans for any goal.', tag: 'Goals', tagColor: 'bg-pink-100 text-pink-700' },
  ]

  const steps = [
    { n: '01', icon: '👤', title: 'Create Profile', desc: 'Sign up with your health details in 30 seconds.', pts: ['Personal health data', 'Dietary preferences', 'Health goals'] },
    ...
    { n: '03', icon: '📈', title: 'Track & Improve', desc: 'Chat with AI, log meals, transform your health.', pts: ['AI consultation', 'Progress tracking', 'Daily health tips'] },
  ]
  ...
  const testimonials = [
    ...
    { name: 'Dr. Anita Patel', role: 'Doctor, Ahmedabad', av: 'AP', avColor: 'from-purple-500 to-violet-500', text: 'I recommend NutriAI to patients. Evidence-based nutrition advice. Truly impressive!', stars: 5, result: '95%', resultColor: 'bg-purple-100 text-purple-700' },
  ]
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {[['Features', '#features'], ['How it Works', '#how'], ['Testimonials', '#testi']].map(([l, h]) => (
              <a key={l} href={h} className="text-sm font-medium text-gray-500 hover:text-green-600 transition-colors relative group">
                {l}
              ...
              <div className="relative">
                <div className="bg-white rounded-3xl shadow-2xl p-6 border border-gray-100" style={{ animation: 'float 4s ease-in-out infinite' }}>
                  <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-50">
                    <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow-md"><span className="text-lg">🤖</span></div>
                    <div>
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4 mb-4">
                    ...
                    {[['🌅', 'Breakfast', 'Oats + Banana', '320'], ['☀️', 'Lunch', 'Dal Rice + Salad', '580'], ['🌙', 'Dinner', 'Khichdi + Dahi', '400']].map(([ic, t, f, c]) => (
                      <div key={t} className="flex items-center justify-between bg-white rounded-xl px-3 py-2.5 shadow-sm mb-1.5 last:mb-0">
                        <div className="flex items-center gap-2">
                          <span>{ic}</span>
                          <div><p className="text-xs text-gray-400">{t}</p><p className="text-xs font-semibold text-gray-700">{f}</p></div>
                        </div>
                ...
                <div className="absolute -top-5 -right-4 bg-white rounded-2xl shadow-xl px-3 py-2.5 flex items-center gap-2 border border-gray-50" style={{ animation: 'float 3s ease-in-out infinite', animationDelay: '0.5s' }}>
                  ...
                  <div><p className="text-xs font-extrabold text-gray-900">4.9 / 5</p><p className="text-xs text-gray-400">10K+ users</p></div>
                </div>
      ...
      <section className="py-24 bg-gray-900 text-white relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-16">
            {[
              { i: '🇮🇳', t: 'Built for India', d: 'Understands Indian cuisine and food culture.' },
              { i: '🧬', t: 'Science-Based', d: 'Peer-reviewed nutritional science.' },
              { i: '🔒', t: 'Privacy First', d: 'JWT encrypted, never shared.' },
              { i: '⚡', t: 'Instant Results', d: 'Personalized plan in 60 seconds.' },
            ].map((b, i) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden" style={{ animation: 'fadeUp 0.35s ease' }}>
            ...
            <div className="relative p-6 text-white overflow-hidden" style={{ background: 'linear-gradient(135deg,#10b981,#059669)' }}>
              ...
              <button onClick={() => setModal(null)} className="absolute top-4 right-4 w-8 h-8 bg-white/20 hover:bg-white/30 rounded-xl flex items-center justify-center text-sm font-bold transition-colors">✕</button>
              <p className="text-2xl mb-1">🥗</p>
              <h2 className="text-xl font-extrabold">{modal === 'login' ? 'Welcome Back!' : '🔑 Forgot Password'}</h2>
              <p className="text-green-100 text-sm">{modal === 'login' ? 'Sign in to NutriAI' : 'Reset your password'}</p>
            </div>
            ...
            <div className="p-6">
              ...
              {modal === 'login' && (
                <>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="relative">
                      <input
                        name="password" value={loginForm.password} onChange={upL}
                  ...
                  <button onClick={doLogin} disabled={loading}
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3.5 rounded-2xl hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] transition-all disabled:opacity-50">
                  ...
                  <p className="text-center text-gray-400 text-sm mt-4">
                    New to NutriAI?{' '}
                    <button onClick={() => { setModal(null); navigate('/register') }} className="text-green-600 font-bold hover:text-green-700">Create free account</button>
                  </p>
                </>
              )}
              ...
              {modal === 'forgot' && !success && (
                <>
                  ...
                  <button onClick={doForgot} disabled={loading}
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3.5 rounded-2xl hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] transition-all disabled:opacity-50">
                    {loading ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Sending...</span> : '📧 Send Reset Link'}
                  </button>
                  <button onClick={() => { setModal('login'); setError(''); setSuccess('') }}
                    className="w-full text-center text-gray-400 text-sm mt-3 hover:text-green-600 transition-colors">← Back to Sign In</button>
                </>
              )}
              ...
              {modal === 'forgot' && success && (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-400 text-sm mb-5">Reset link sent to <span className="font-semibold text-gray-700">{forgotEmail}</span></p>
                  <div className="bg-green-50 border border-green-100 rounded-2xl p-4 text-left mb-5">
                    <p className="text-sm text-green-700 font-semibold mb-2">📋 Next steps:</p>
                    <ol className="text-xs text-gray-500 space-y-1 list-decimal list-inside">
                      <li>Check your email inbox (also spam)</li>
                  ...
                  <button onClick={() => { setModal('login'); setError(''); setSuccess(''); setForgotEmail('') }}
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3 rounded-2xl hover:scale-[1.02] transition-all">
                    Back to Sign In
                  </button>
                </div>
              )}
